...
#import "ZYVideoReverse.h"

@implementation ZYVideoReverse {
...
//    __weak typeof(self) weakSelf = self;
    ...
    int numSamplesInPass = 100;
    // write to output in <numSamplesInPass> frame increments
    ...
    dispatch_async(reverseQueue, ^{
        ...
        self->assetReader = [[AVAssetReader alloc] initWithAsset:inputAsset error:&error];
        ...
        self->assetReaderOutput = [AVAssetReaderTrackOutput assetReaderTrackOutputWithTrack:videoTrack outputSettings:self.readerOutputSettings];
        self->assetReaderOutput.supportsRandomAccess = YES;
        ...
        int localCount = 0;
        while((sample = [self->assetReaderOutput copyNextSampleBuffer])) {
        ...
        [self->assetWriterInput setExpectsMediaDataInRealTime:NO];
        [self->assetWriterInput setTransform:[videoTrack preferredTransform]];
        ...
        [self->assetWriter startSessionAtSourceTime:initEventTime];
        ...
        for (NSInteger z=passDicts.count-1; z>=0; z--) {
            ...
            [self->assetReaderOutput resetForReadingTimeRanges:localRanges];
            if (self.showDebug)
...
- (void)conveyErrorWithMessage:(NSString*)message {
    ...
    if (self.delegate != nil && [self.delegate respondsToSelector:@selector(didFinishReverse:withError:)]) {
        ...
        NSError *error = [NSError errorWithDomain:@"ZYVideoReverse" code:-1 userInfo:userInfo];
        dispatch_async(dispatch_get_main_queue(),^{
